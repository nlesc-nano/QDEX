#!/bin/bash
# Reproduce the CdSe 2.0 nm model sweep reported in
...
run pbe --qp_gap pbe
run sgw-resta --qp_gap sgw-resta
run sgw-resta_eps2.4 --qp_gap sgw-resta --eps-out 2.4
run sgw-resta-pure --qp_gap sgw-resta-pure
run sgw-dim --qp_gap sgw-dim
run evgw-resta --qp_gap evgw-resta
run qsgw-resta --qp_gap qsgw-resta
run qsgw-dim --qp_gap qsgw-dim
run sgw_sbse --qp_gap sgw --kernel sbse
run diag_bse --excitation-mode diagonal_bse
run indep_qp --excitation-mode independent_qp
run as50 --nhomos 50 --nlumos 50
run k_dim --kernel dim
run k_sbse --kernel sbse
run k_mnok --kernel bse
run k_xsresta --kernel xs-resta
